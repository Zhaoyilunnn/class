...
class DqcMapLayout(TransformationPass):
    ...
    **References:**
    ...
    def run(self, dag):
        ...
        """
        # Use initial layouts generated by dqcmap mapper as starting layouts of sabre
        if self._sabre_starting_layouts is not None:
            layouts = []
            for start_layout in self._sabre_starting_layouts:
                layout = Layout(
                    {phys: dag.qubits[i] for i, phys in enumerate(start_layout)}
                )
                layouts.append(layout)
            self.property_set["sabre_starting_layouts"] = layouts
