...
class KLMapper(BaseMapper):
            ...
            while improved and local_search_count < 5:
                ...
                for ctrl1 in range(self.n_controllers):
                    for ctrl2 in range(ctrl1 + 1, self.n_controllers):
                        new_mapping, new_score = self.kernighan_lin_pass(
                            current_mapping, ctrl1, ctrl2
                        )
                ...
                if new_score >= current_score:
                    ...
                    local_mapping = self.local_search(
                        current_mapping, depth=3, iterations=100
                    )
    ...
    def kernighan_lin_pass(
        self, mapping: List[int], ctrl1: int, ctrl2: int
    ) -> Tuple[List[int], int]:
        partition1 = [
            q for q in range(self.n_logical) if self.get_controller(mapping[q]) == ctrl1
        ]
        partition2 = [
            q for q in range(self.n_logical) if self.get_controller(mapping[q]) == ctrl2
        ]
        ...
        while unmoved:
            ...
            current_mapping[a], current_mapping[b] = (
                current_mapping[b],
                current_mapping[a],
            )
